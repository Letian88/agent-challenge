...
团队 SHALL 记录仓库与群公告给出的开放时间为 2026-09-18 20:00，同时 MUST 记录题包 `roundStartAt`、`roundEndAt` 均为空且正文未给出截止时间。正式截止里程碑和提交次数限制 MUST 等组织方确认，不得从评测耗时反推。

#### Scenario: 安排最终提交
- **WHEN** 组织方尚未发布可追溯的截止时间
- **THEN** 最终提交时间保持待确认，只推进不依赖截止日期的准备工作
